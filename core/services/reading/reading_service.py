...
from core.ai.llm_text_content_generator import LLMTextContentGenerator
from core.constants import DAILY_LIMIT_PER_THEME
from core.database import text_log
...
from handlers.ui.ui_main import reading


async def has_exceeded_daily_limit(session, uid: int, theme: str) -> bool:
    """Проверяет, превышен ли дневной лимит генерации по теме для пользователя."""
    count = await text_log.count_generated_today(session, uid, theme)
...
async def prepare_first_question(uid: int, category: str, theme: str) -> ReadingState:
    ...
    async with AsyncSessionLocal.begin() as session:
        age = await get_user_age(uid, session)
